...
func (s Signature) VerifyByOperators(data MessageSignature, domain DomainType, sigType SignatureType, operators []*ValidatorShare) error {
	// decode sig
	sign := &bls.Sign{}
	...
	for _, id := range data.GetSigners() {
		...
		for _, n := range operators {
			if id == n.OperatorID {
				pk := bls.PublicKey{}
				if err := pk.Deserialize(n.OperatorPubKey); err != nil {
