...
//go:generate go run github.com/ferranbt/fastssz/sszgen --path operator.go --include ./cluster_id.go --exclude-objs OperatorID

//go:generate rm -f ./share_encoding.go
//go:generate go run github.com/ferranbt/fastssz/sszgen --path share.go --include $GOPATH/pkg/mod/github.com/attestantio/go-eth2-client@v0.19.7/spec/phase0,./operator.go,./messages.go,./signer.go,./domain_type.go
...
//go:generate go run github.com/ferranbt/fastssz/sszgen --path beacon_types.go --include $GOPATH/pkg/mod/github.com/attestantio/go-eth2-client@v0.19.7/spec/phase0 --exclude-objs BeaconNetwork,BeaconRole,CommitteeDuty
