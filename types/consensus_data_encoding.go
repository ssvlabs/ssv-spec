...
// Hash: 0bd5fb1106cc13a7ce7d27df5c7cee4e67dc5e6c77dac2975fe4ec6e139e599b
package types
...
func (cd *ConsensusData) MarshalSSZ() ([]byte, error) {
	dSSZ := &dutySSZ{
		Type:                    uint8(int8(cd.Duty.Type)),
		PubKey:                  cd.Duty.PubKey,
		Slot:                    cd.Duty.Slot,
		ValidatorIndex:          cd.Duty.ValidatorIndex,
		CommitteeIndex:          cd.Duty.CommitteeIndex,
		CommitteeLength:         cd.Duty.CommitteeLength,
		CommitteesAtSlot:        cd.Duty.CommitteesAtSlot,
		ValidatorCommitteeIndex: cd.Duty.ValidatorCommitteeIndex,
	}
	...
	return ssz.MarshalSSZ(&consensusDataSSZ{
		Duty: dSSZ,
		Data: marshalSSZ,
	})
}
...
func (cd *ConsensusData) UnmarshalSSZ(buf []byte) error {
	var cdSSZ consensusDataSSZ
	if err := cdSSZ.UnmarshalSSZ(buf); err != nil {
		return err
	}

	cd.Duty = &Duty{}
	cd.Duty.Type = BeaconRole(int8(cdSSZ.Duty.Type))
	cd.Duty.PubKey = cdSSZ.Duty.PubKey
	cd.Duty.Slot = cdSSZ.Duty.Slot
	cd.Duty.ValidatorIndex = cdSSZ.Duty.ValidatorIndex
	cd.Duty.CommitteeIndex = cdSSZ.Duty.CommitteeIndex
	cd.Duty.CommitteeLength = cdSSZ.Duty.CommitteeLength
	cd.Duty.CommitteesAtSlot = cdSSZ.Duty.CommitteesAtSlot
	cd.Duty.ValidatorCommitteeIndex = cdSSZ.Duty.ValidatorCommitteeIndex
	...
	case BNRoleProposer:
		var blockData altair.BeaconBlock
...
// MarshalSSZ ssz marshals the consensusDataSSZ object
func (c *consensusDataSSZ) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(c)
}

// MarshalSSZTo ssz marshals the consensusDataSSZ object to a target array
func (c *consensusDataSSZ) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(101)
...
// UnmarshalSSZ ssz unmarshals the consensusDataSSZ object
func (c *consensusDataSSZ) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 101 {
		return ssz.ErrSize
	...
	if err = c.Duty.UnmarshalSSZ(buf[0:97]); err != nil {
		return err
	}

	// Offset (1) 'Data'
	if o1 = ssz.ReadOffset(buf[97:101]); o1 > size {
...
// SizeSSZ returns the ssz encoded size in bytes for the consensusDataSSZ object
func (c *consensusDataSSZ) SizeSSZ() (size int) {
	size = 101
...
// HashTreeRoot ssz hashes the consensusDataSSZ object
func (c *consensusDataSSZ) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(c)
}

// HashTreeRootWith ssz hashes the consensusDataSSZ object with a hasher
func (c *consensusDataSSZ) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
...
func (c *contributionEntries) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if len(c.SyncCommitteeContribution) > 7 {
