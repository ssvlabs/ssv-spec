...
// Hash: 6cf11cc9ab16c4f58bd8c8a4ce148571772f47346ebef4fbfe670ae33b86b00d
// Version: 0.1.3
package types
...
func (a *AggregatorCommitteeConsensusData) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (1) 'Aggregators'
	if size := len(a.Aggregators); size > 256 {
		err = ssz.ErrListTooBigFn("AggregatorCommitteeConsensusData.Aggregators", size, 256)
		return
	}
	...
	if size := len(a.Attestations); size > 64 {
		err = ssz.ErrListTooBigFn("AggregatorCommitteeConsensusData.Attestations", size, 64)
		return
	}
	{
	...
	if size := len(a.SyncCommitteeSubnets); size > 4 {
		err = ssz.ErrListTooBigFn("AggregatorCommitteeConsensusData.SyncCommitteeSubnets", size, 4)
		return
	}
