...
// Hash: ebadfdae0b1d65cdd03ffd3866ad491d5db74746e1f9ca805904b683c88e1a6c
// Version: 0.1.3
package types
...
// MarshalSSZ ssz marshals the ProposerConsensusData object
func (p *ProposerConsensusData) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(p)
}

// MarshalSSZTo ssz marshals the ProposerConsensusData object to a target array
func (p *ProposerConsensusData) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(16)

	// Offset (0) 'Duty'
	dst = ssz.WriteOffset(dst, offset)
	offset += p.Duty.SizeSSZ()

	// Field (1) 'Version'
	dst = ssz.MarshalUint64(dst, uint64(p.Version))

	// Offset (2) 'DataSSZ'
	dst = ssz.WriteOffset(dst, offset)

	// Field (0) 'Duty'
	if dst, err = p.Duty.MarshalSSZTo(dst); err != nil {
		return
	}

	// Field (2) 'DataSSZ'
	if size := len(p.DataSSZ); size > 8388608 {
		err = ssz.ErrBytesLengthFn("ProposerConsensusData.DataSSZ", size, 8388608)
		return
	}
	dst = append(dst, p.DataSSZ...)

	return
}

// UnmarshalSSZ ssz unmarshals the ProposerConsensusData object
func (p *ProposerConsensusData) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 16 {
		return ssz.ErrSize
	...
	// Field (1) 'Version'
	p.Version = spec.DataVersion(ssz.UnmarshallUint64(buf[4:12]))
	...
	{
		buf = tail[o0:o2]
		if err = p.Duty.UnmarshalSSZ(buf); err != nil {
			return err
		}
	}
	...
	// Field (2) 'DataSSZ'
	{
		...
		if cap(p.DataSSZ) == 0 {
			p.DataSSZ = make([]byte, 0, len(buf))
		}
		p.DataSSZ = append(p.DataSSZ, buf...)
	}
	return err
}

// SizeSSZ returns the ssz encoded size in bytes for the ProposerConsensusData object
func (p *ProposerConsensusData) SizeSSZ() (size int) {
	size = 16
...
// HashTreeRoot ssz hashes the ProposerConsensusData object
func (p *ProposerConsensusData) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(p)
}

// HashTreeRootWith ssz hashes the ProposerConsensusData object with a hasher
func (p *ProposerConsensusData) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	indx := hh.Index()

	// Field (0) 'Duty'
	if err = p.Duty.HashTreeRootWith(hh); err != nil {
		return
	}

	// Field (1) 'Version'
	hh.PutUint64(uint64(p.Version))

	// Field (2) 'DataSSZ'
	{
		elemIndx := hh.Index()
		byteLen := uint64(len(p.DataSSZ))
		if byteLen > 8388608 {
...
// GetTree ssz hashes the ProposerConsensusData object
func (p *ProposerConsensusData) GetTree() (*ssz.Node, error) {
