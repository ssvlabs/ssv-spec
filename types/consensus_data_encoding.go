...
// Hash: 37b1def58b3830e7492feee27401c2c88bdc9c0405af7be21a5fafcea5a951f6
// Version: 0.1.3
package types
...
func (a *AggregatorCommitteeConsensusData) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(a.AggregatorsCommitteeIndexes); size > 3000 {
		err = ssz.ErrListTooBigFn("AggregatorCommitteeConsensusData.AggregatorsCommitteeIndexes", size, 3000)
		return
	}
	...
	if size := len(a.Attestations); size > 3000 {
		err = ssz.ErrListTooBigFn("AggregatorCommitteeConsensusData.Attestations", size, 3000)
		return
	}
	{
	...
	if size := len(a.Contributors); size > 256 {
		err = ssz.ErrListTooBigFn("AggregatorCommitteeConsensusData.Contributors", size, 256)
		return
	}
	...
	if size := len(a.SyncCommitteeSubnets); size > 256 {
		err = ssz.ErrListTooBigFn("AggregatorCommitteeConsensusData.SyncCommitteeSubnets", size, 256)
		return
	}
	...
	if size := len(a.SyncCommitteeContributions); size > 256 {
		err = ssz.ErrListTooBigFn("AggregatorCommitteeConsensusData.SyncCommitteeContributions", size, 256)
		return
	}
...
func (a *AggregatorCommitteeConsensusData) UnmarshalSSZ(buf []byte) error {
	...
	{
		buf = tail[o3:o4]
		num, err := ssz.DecodeDynamicLength(buf, 3000)
		if err != nil {
			return err
		}
	...
	{
		buf = tail[o4:o5]
		num, err := ssz.DivideInt2(len(buf), 112, 256)
		if err != nil {
			return err
		}
	...
	{
		buf = tail[o5:o6]
		num, err := ssz.DivideInt2(len(buf), 8, 256)
		if err != nil {
			return err
		}
	...
	{
		buf = tail[o6:]
		num, err := ssz.DivideInt2(len(buf), 160, 256)
		if err != nil {
			return err
		}
...
func (a *AggregatorCommitteeConsensusData) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	...
	// Field (5) 'SyncCommitteeSubnets'
	{
		...
		hh.MerkleizeWithMixin(subIndx, numItems, ssz.CalculateLimit(256, numItems, 8))
	}
