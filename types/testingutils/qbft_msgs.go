...
var MultiSignQBFTMsg = func(sks []*bls.SecretKey, ids []types.OperatorID, msg *qbft.Message, input *qbft.Data) *qbft.SignedMessage {
	...
	for i, sk := range sks {
		if signed == nil {
			signed = SignQBFTMsg(sk, ids[i], msg, input)
		} else {
			if err := signed.Aggregate(SignQBFTMsg(sk, ids[i], msg, input)); err != nil {
				panic(err.Error())
			}
		}
	}
...
var SignQBFTMsg = func(sk *bls.SecretKey, id types.OperatorID, msg *qbft.Message, input *qbft.Data) *qbft.SignedMessage {
	domain := types.PrimusTestnet
	sigType := types.QBFTSignatureType
	msg.InputRoot = input.Root
