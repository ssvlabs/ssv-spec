...
// Hash: 237d6cf4d9e82bbe340326f030a92ebe4c39eb273eded5e9dcc62313acc69f4a
// Version: 0.1.3
package types
...
func (s *SignedSSVMessage) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	for ii := 0; ii < len(s.Signature); ii++ {
		if size := len(s.Signature[ii]); size > 256 {
			err = ssz.ErrBytesLengthFn("SignedSSVMessage.Signature[ii]", size, 256)
			return
		}
	...
	// Field (3) 'FullData'
	if size := len(s.FullData); size > 6291829 {
		err = ssz.ErrBytesLengthFn("SignedSSVMessage.FullData", size, 6291829)
		return
	}
