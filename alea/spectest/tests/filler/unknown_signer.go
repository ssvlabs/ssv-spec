...
func UnknownSigner() *tests.MsgProcessingSpecTest {
	...
	msgs := []*alea.SignedMessage{
		testingutils.SignAleaMsg(testingutils.Testing4SharesSet().Shares[2], types.OperatorID(5), &alea.Message{
			...
			Data:       testingutils.FillerDataBytes(tests.Entries, tests.Priorities, tests.AggregatedMsgBytesList(types.OperatorID(1), alea.FirstPriority), types.OperatorID(1)),
		}),
	}
	return &tests.MsgProcessingSpecTest{
		Name:          "filler unknown proposal signer",
		Pre:           pre,
