...
func (i *Instance) uponABAInit(signedABAInit *SignedMessage) error {
	...
	// old message -> ignore
	if abaInitData.ACSRound < i.State.HBBFTState.GetRound() {
		return nil
	}
	if abaInitData.Round < i.State.HBBFTState.GetCurrentABAState().Round {
		return nil
	}
	// if future round -> intialize future state
	if abaInitData.ACSRound > i.State.HBBFTState.GetRound() {
		i.State.HBBFTState.InitMaps(abaInitData.ACSRound)
	}
	if abaInitData.Round > i.State.HBBFTState.GetABAState(abaInitData.ACSRound).Round {
		i.State.HBBFTState.GetABAState(abaInitData.ACSRound).InitializeRound(abaInitData.Round)
	}

	abaState := i.State.HBBFTState.GetABAState(abaInitData.ACSRound)
	...
	for _, vote := range []byte{0, 1} {
		if !abaState.sentInit(abaInitData.Round, vote) && abaState.countInit(abaInitData.Round, vote) >= i.State.Share.PartialQuorum {
			...
			// send INIT
			initMsg, err := CreateABAInit(i.State, i.config, vote, abaInitData.Round, abaInitData.ACSRound)
			if err != nil {
	...
	for _, vote := range []byte{0, 1} {
		...
		if !abaState.sentAux(abaInitData.Round, vote) && abaState.countInit(abaInitData.Round, vote) >= i.State.Share.Quorum {
			...
			// sends AUX(b)
			auxMsg, err := CreateABAAux(i.State, i.config, vote, abaInitData.Round, abaInitData.ACSRound)
			if err != nil {
