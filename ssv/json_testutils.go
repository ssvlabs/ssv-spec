...
func (pcs *State) MarshalJSON() ([]byte, error) {
	...
	if pcs.StartingDuty != nil {
		if validatorDuty, ok := pcs.StartingDuty.(*types.ValidatorDuty); ok {
			...
			return nil, errors.New("can't marshal because BaseRunner.State.StartingDuty isn't ValidatorDuty, CommitteeDuty, or CBSigningDuty")
		}
	}
...
// CBSigningRunner
func (r *CBSigningRunner) Encode() ([]byte, error) {
	return json.Marshal(r)
}

func (r *CBSigningRunner) Decode(data []byte) error {
	return json.Unmarshal(data, &r)
}

func (r *CBSigningRunner) GetRoot() ([32]byte, error) {
	marshaledRoot, err := r.Encode()
	if err != nil {
		return [32]byte{}, errors.Wrap(err, "could not encode CBSigningRunner")
	}
	ret := sha256.Sum256(marshaledRoot)
	return ret, nil
}
