...
func InvalidSignedMessageNoSigners() tests.SpecTest {
	...
	multiSpecTest := tests.NewMultiMsgProcessingSpecTest(
		...
		testdoc.PostConsensusInvalidSignedMsgNoSignersDoc,
	...
	for _, version := range testingutils.SupportedAggregatorVersions {
		multiSpecTest.Tests = append(multiSpecTest.Tests, []*tests.MsgProcessingSpecTest{
			...
			},
			{
				...
				Messages: []*types.SignedSSVMessage{
					noSigners(testingutils.SignedSSVMessageWithSigner(1, ks.OperatorKeys[1], testingutils.SSVMsgAggregatorCommittee(ks, nil, testingutils.PostConsensusAggregatorCommitteeMixedMsg(ks.Shares[1], 0, version, ks)))),
				},
				DontStartDuty:     true,
				ExpectedErrorCode: expectedErrorCode,
			},
	...
	multiSpecTest.Tests = append(multiSpecTest.Tests, &tests.MsgProcessingSpecTest{
		...
		Messages: []*types.SignedSSVMessage{
			noSigners(testingutils.SignedSSVMessageWithSigner(1, ks.OperatorKeys[1], testingutils.SSVMsgEnvelopeProposer(nil, testingutils.PostConsensusEnvelopeProposerMsg(ks.Shares[1], 0)))),
		},
		DontStartDuty:     true,
		ExpectedErrorCode: expectedErrorCode,
	})
