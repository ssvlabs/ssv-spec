...
func TooFewRoots() tests.SpecTest {
	...
	multiSpecTest.Tests = append(multiSpecTest.Tests, &tests.MsgProcessingSpecTest{
		...
		Messages: []*types.SignedSSVMessage{
			testingutils.SignedSSVMessageWithSigner(1, ks.OperatorKeys[1], testingutils.SSVMsgEnvelopeProposer(nil, testingutils.PostConsensusEnvelopeProposerTooFewRootsMsg(ks.Shares[1], 1))),
		},
		DontStartDuty:     true,
		ExpectedErrorCode: errCode,
	})
