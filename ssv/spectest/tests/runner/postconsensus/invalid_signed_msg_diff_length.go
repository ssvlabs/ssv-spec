...
func InvalidSignedMessageDifferentLength() tests.SpecTest {
	...
	multiSpecTest.Tests = append(multiSpecTest.Tests, &tests.MsgProcessingSpecTest{
		...
		Messages: []*types.SignedSSVMessage{
			differentLength(testingutils.SignedSSVMessageWithSigner(1, ks.OperatorKeys[1], testingutils.SSVMsgEnvelopeProposer(nil, testingutils.PostConsensusEnvelopeProposerMsg(ks.Shares[1], 0)))),
		},
		DontStartDuty:     true,
		ExpectedErrorCode: expectedErrorCode,
	})
