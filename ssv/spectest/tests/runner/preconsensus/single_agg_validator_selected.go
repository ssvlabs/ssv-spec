...
// SingleAggregatorValidatorSelected ensures a single-validator quorum that gets selected results in starting QBFT
func SingleAggregatorValidatorSelected() tests.SpecTest {
	...
	multiSpecTest := tests.NewMultiMsgProcessingSpecTest(
		"pre consensus single aggregator validator selected",
		testdoc.PreConsensusAggregatorCommitteeAggregatorSingleValidatorSelectedDoc,
	...
	for _, version := range testingutils.SupportedAggregatorVersions {
		...
		multiSpecTest.Tests = append(multiSpecTest.Tests, &tests.MsgProcessingSpecTest{
			Name:   fmt.Sprintf("single aggregator validator quorum starts consensus (%s)", version.String()),
