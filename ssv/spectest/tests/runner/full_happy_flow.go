	...
	multiSpecTest := &tests.MultiMsgProcessingSpecTest{
		...
		Tests: []*tests.MsgProcessingSpecTest{
			...
			},
			{
				...
				PostDutyRunnerStateRoot: fullHappyFlowCBSigningSC().Root(),
				PostDutyRunnerState:     fullHappyFlowCBSigningSC().ExpectedState,
				OutputMessages: []*types.PartialSignatureMessages{
					testingutils.PreConsensusCBSigningMsg(ks.Shares[1], 1), // broadcasts when starting a new duty
				},
				// commit boost signing duty does not broadcast to beacon
			},
		},
	}
