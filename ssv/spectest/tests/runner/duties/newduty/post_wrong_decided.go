...
func PostWrongDecided() *MultiStartNewRunnerDutySpecTest {
	...
	decideWrong := func(r ssv.Runner, duty *types.Duty) ssv.Runner {
		...
		r.GetBaseRunner().QBFTController.StoredInstances.AddNewInstanceAtPosition(1, r.GetBaseRunner().State.RunningInstance)
		...
		r.GetBaseRunner().QBFTController.StoredInstances.AddNewInstanceAtPosition(0, higherDecided)
