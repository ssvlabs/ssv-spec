...
func InvalidJustification() tests.SpecTest {
	...
	setZeroSignerAndPrepareMsg := func(obj *types.ConsensusData, id []byte) *types.SignedSSVMessage {
		if len(obj.PreConsensusJustifications) > 0 {
			for _, msg := range obj.PreConsensusJustifications[0].Messages {
				msg.Signer = 0
			}
		}
		...
		Tests: []*tests.MsgProcessingSpecTest{
			{
				...
				Messages: []*types.SignedSSVMessage{
					setZeroSignerAndPrepareMsg(testingutils.TestContributionProofWithJustificationsConsensusData(ks), testingutils.SyncCommitteeContributionMsgID),
				},
			...
			},
			{
				...
				Messages: []*types.SignedSSVMessage{
					setZeroSignerAndPrepareMsg(testingutils.TestSelectionProofWithJustificationsConsensusData(ks), testingutils.AggregatorMsgID),
				},
			...
			},
			{
				...
				Messages: []*types.SignedSSVMessage{
					setZeroSignerAndPrepareMsg(testingutils.TestProposerWithJustificationsConsensusDataV(ks, spec.DataVersionDeneb), testingutils.ProposerMsgID),
				},
			...
			},
			{
				...
				Duty:   testingutils.TestingProposerDutyV(spec.DataVersionDeneb),
				Messages: []*types.SignedSSVMessage{
					setZeroSignerAndPrepareMsg(testingutils.TestProposerBlindedWithJustificationsConsensusDataV(ks, spec.DataVersionDeneb), testingutils.ProposerMsgID),
				},
			...
			},
			{
				...
				Messages: []*types.SignedSSVMessage{
					setZeroSignerAndPrepareMsg(testingutils.TestSyncCommitteeConsensusData, testingutils.SyncCommitteeMsgID),
				},
