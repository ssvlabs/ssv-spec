...
func ShuffledHappyFlowDutiesWithTheSameValidators() tests.SpecTest {
				...
				tests = append(tests, []*committee.CommitteeSpecTest{
					{
						...
						Input:                  testingutils.CommitteeInputForDutiesWithShuffle(numSequencedDuties, numValidators, 0, true, version),
					...
					},
					{
						...
						Committee:              testingutils.BaseCommitteeWithCreatorFieldsFromRunner(ksMap, testingutils.CommitteeRunnerWithShareMap(shareMap).(*ssv.CommitteeRunner)),
						Input:                  testingutils.CommitteeInputForDutiesWithShuffle(numSequencedDuties, 0, numValidators, true, version),
					...
					},
					{
						...
						Committee:              testingutils.BaseCommitteeWithCreatorFieldsFromRunner(ksMap, testingutils.CommitteeRunnerWithShareMap(shareMap).(*ssv.CommitteeRunner)),
						Input:                  testingutils.CommitteeInputForDutiesWithShuffle(numSequencedDuties, numValidators, numValidators, true, version),
	...
	multiSpecTest := committee.NewMultiCommitteeSpecTest(
		...
		"Tests complete happy flow execution for sequences of duties with shuffled input messages while preserving order between duty messages for the same validators",
		tests,
	)
