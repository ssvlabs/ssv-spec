...
func (r *AggregatorRunner) ProcessPostConsensus(signedMsg *types.PartialSignatureMessages) error {
	...
	for _, root := range roots {
		sig, err := r.GetState().ReconstructBeaconSig(r.GetState().PostConsensusContainer, root,
			r.GetShare().ValidatorPubKey[:])
		if err != nil {
			// If the reconstructed signature verification failed, fall back to verifying each partial signature
			for _, root := range roots {
...
func (r *AggregatorRunner) executeDuty(duty types.Duty) error {
	...
	msg, err := r.BaseRunner.signBeaconObject(r, duty.(*types.BeaconDuty), types.SSZUint64(duty.DutySlot()), duty.DutySlot(),
