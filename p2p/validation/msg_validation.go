...
func validateConsensusMsg(runner ssv.Runner, signedMsg *types.SignedSSVMessage) error {
	...
	*/
	isDecided, err := qbft.IsDecidedMsg(contr.SharedValidator, signedMsg)
	if err != nil {
		return err
	}
	if isDecided {
		return qbft.ValidateDecided(contr.GetConfig(), signedMsg, contr.SharedValidator)
	}
	...
	if msg.Height > contr.Height {
		return validateFutureMsg(contr.GetConfig(), signedMsg, contr.SharedValidator)
	}
