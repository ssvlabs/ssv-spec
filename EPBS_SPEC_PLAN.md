...
*Status (audit 2026-07-07): the type layer described below already landed on `epbs-gloas-types` (both `types/gloas` commits). **Done:** container family + round-trip/wire tests, `DataVersionGloas` + `BuilderIndexSelfBuild`, flat `GloasBeaconVote` / `EnvelopeConsensusData` + encoding+root vectors + fixtures, sszgen directives, positional HTR (intended per the revised D9 — not a gap). **Remaining:** fork machinery (do first — blocks PRs 3–7); `ProposerConsensusData.Validate()` Gloas gate + type-level unit test; `maxmsgsize` (D8); error codes (D10); container-level encoding tests; and hold the §4/§6 signing-root goldens (revised D9). Full spec below.*
