...
- `ProposerConsensusData.GetBlockData()` stays pre-Gloas (it returns `*api.VersionedProposal`, which cannot carry Gloas). **Gloas block validation lives in the `ssv` value-check, not in `types` — design decision 2026-07-07 (Option A).** Rationale: `types/gloas` is a delete-when-upstream-ships subpackage, so the base `types` package must not import it for concrete decoding; keeping the gloas coupling in `ssv` matches the committed `EnvelopeConsensusData` design ("opaque `DataSSZ`, no gloas import, decoded by the runner + value check") and gives one model for both consensus-data types. Concretely: `ProposerConsensusData.Validate()` returns `nil` for `Version > spec.DataVersionFulu` (opaque; **no gloas import in `types`** — without this gate the current `GetBlockData` path errors `UnknownBlockVersion` on Gloas), and `ProposerValueCheckF` (`ssv/value_check.go`) decodes `gloas.BeaconBlock` (→ `UnmarshalSSZErrorCode` on failure), pins `block.Slot == duty.Slot` (→ `ProposerBlockSlotMismatchErrorCode`, mirroring §6's duty-slot match — SIP-94 §4), and takes the block slot for the slashing check — where the QBFT value is actually gated during consensus. `ProcessConsensus` / `expectedPostConsensusRootsAndDomain` / `ProcessPostConsensus` decode the Gloas block the same way (sign its root under `DomainProposer`; submit via `SubmitGloasBeaconBlock`). Tradeoff (accepted): intra-type asymmetry — pre-Gloas blocks are decode-checked in `Validate()`, Gloas in the value-check; the value-check is what gates the value during consensus, and Gloas can't use `Validate()`'s `api.VersionedProposal` path anyway. End-state this points toward: `ProposerConsensusData` eventually as thin as `EnvelopeConsensusData` (all block validation in `ssv`), deferred as a pre-Gloas-path refactor. The `Validate()` opaque gate and the `ProposerValueCheckF` Gloas decode land **together** (both halves of Option A — the value check must decode `gloas.BeaconBlock` to accept a Gloas value); only the proposer *runner*'s Gloas handling (`ProcessConsensus` etc.) remains PR 4.
...
*Status (audit 2026-07-07, updated): the type layer below already landed on `epbs-gloas-types` (both `types/gloas` commits), and the fork machinery has since landed too. **Done:** container family + round-trip/wire tests, `DataVersionGloas` + `BuilderIndexSelfBuild`, flat `GloasBeaconVote` / `EnvelopeConsensusData` + encoding+root vectors + fixtures, sszgen directives, positional HTR (intended per the revised D9 — not a gap), **fork machinery** (`ForkEpochGloas` + Gloas arms in `VersionBySlot` / `VersionByEpoch` / `TestingDuty{Epoch,Slot}V`), **Option A block validation** (`ProposerConsensusData.Validate()` opaque for `Version>Fulu`, no gloas import; `ProposerValueCheckF` decodes `gloas.BeaconBlock` → `UnmarshalSSZErrorCode`, and pins `block.Slot == duty.Slot` → `ProposerBlockSlotMismatchErrorCode`) + **Go unit test** covering both halves (`ssv/value_check_gloas_test.go`). **Remaining:** `maxmsgsize` (D8); container-level encoding tests; and the **anchor-facing generated vector** for the Gloas value check — deferred pending risk-#6 anchor coordination (the `valcheck` / `proposerconsensusdata` prefixes are anchor-consumed, so Gloas fixtures there run anchor's code) + D9 for any root-asserting fixture. (The earlier `ssv/spectest` `unknown beacon role` panic was a **false alarm** — a stale gitignored `tests.json` with pre-consolidation roles; `make generate-jsons` regenerates it and the suite passes.) Error codes (D10): the Gloas decode reuses `UnmarshalSSZErrorCode`; the §4 slot-pin adds `ProposerBlockSlotMismatchErrorCode` (one new code); D10's Gloas-vote/envelope/PTC/preferences/VR codes remain per-PR (3–7). Hold the §4/§6 signing-root goldens (revised D9). Full spec below.*
