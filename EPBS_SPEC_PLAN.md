...
*Status (audit 2026-07-07, updated): the type layer below already landed on `epbs-gloas-types` (both `types/gloas` commits), and the fork machinery has since landed too. **Done:** container family + round-trip/wire tests, `DataVersionGloas` + `BuilderIndexSelfBuild`, flat `GloasBeaconVote` / `EnvelopeConsensusData` + encoding+root vectors + fixtures, sszgen directives, positional HTR (intended per the revised D9 — not a gap), **fork machinery** (`ForkEpochGloas` + Gloas arms in `VersionBySlot` / `VersionByEpoch` / `TestingDuty{Epoch,Slot}V`). **Remaining:** `ProposerConsensusData.Validate()` opaque-for-Gloas gate (Option A — return `nil` for `Version>Fulu`, no gloas import; the real `gloas.BeaconBlock` decode lives in `ProposerValueCheckF`, PR 4) + type-level unit test; `maxmsgsize` (D8); container-level encoding tests. Error codes (D10): PR 2 reuses `UnmarshalSSZErrorCode` for the Gloas decode — no new code; D10's Gloas-vote/envelope/PTC/preferences/VR codes are per-PR (3–7). Hold the §4/§6 signing-root goldens (revised D9). Full spec below.*
