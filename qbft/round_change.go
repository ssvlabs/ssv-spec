...
func (i *Instance) uponRoundChange(
	...
	hasQuorumBefore := HasQuorum(i.State.SharedValidator, roundChangeMsgContainer.MessagesForRound(roundChangeMessage.Round))
...
func hasReceivedPartialQuorum(state *State, roundChangeMsgContainer *MsgContainer) (bool, []*types.SignedSSVMessage) {
	...
	return HasPartialQuorum(state.SharedValidator, rc), rc
}
...
func hasReceivedProposalJustificationForLeadingRound(
	...
	if !HasQuorum(state.SharedValidator, roundChanges) {
		return nil, nil, nil
	}
...
func isProposalJustificationForLeadingRound(
	...
	if proposer(state, config, roundChangeMsg.Round) != state.SharedValidator.OwnValidatorShare.OperatorID {
...
func validRoundChangeForDataIgnoreSignature(
	...
	if !signedMsg.CheckSignersInCommittee(state.SharedValidator.Committee) {
	...
	if msg.RoundChangePrepared() {
		...
		for _, pm := range prepareMsgs {
			if err := validSignedPrepareForHeightRoundAndRootVerifySignature(
				...
				msg.DataRound,
				msg.Root,
				state.SharedValidator.Committee); err != nil {
		...
		if !HasQuorum(state.SharedValidator, prepareMsgs) {
...
func validRoundChangeForDataVerifySignature(
	...
	// Verify signature
	if err := config.GetSignatureVerifier().Verify(signedMsg, state.SharedValidator.Committee); err != nil {
...
func CreateRoundChange(state *State, config IConfig, newRound Round, instanceStartValue []byte) (*types.SignedSSVMessage, error) {
	...
	return MessageToSignedSSVMessageWithFullData(msg, state.SharedValidator.OwnValidatorShare.OperatorID, config.GetOperatorSigner(), fullData)
}
