...
func NewController(identifier []byte, share *types.SharedValidator, config IConfig) *Controller {
	return &Controller{
...
func (c *Controller) ProcessMsg(msg *types.SignedSSVMessage) (*types.SignedSSVMessage, error) {
	...
	*/
	isDecided, err := IsDecidedMsg(c.SharedValidator, msg)
	if err != nil {
		return nil, err
	}
	if isDecided {
...
func (c *Controller) addAndStoreNewInstance() *Instance {
	i := NewInstance(c.GetConfig(), c.SharedValidator, c.Identifier, c.Height)
