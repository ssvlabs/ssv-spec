...
// Hash: 0b726e99104bec3b630e443009ae795560a59ac6acdb140ac83f058cc714e37a
// Version: 0.1.3
package qbft
...
func (m *Message) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	for ii := 0; ii < len(m.RoundChangeJustification); ii++ {
		if size := len(m.RoundChangeJustification[ii]); size > 66770 {
			err = ssz.ErrBytesLengthFn("Message.RoundChangeJustification[ii]", size, 66770)
			return
		}
	...
	for ii := 0; ii < len(m.PrepareJustification); ii++ {
		if size := len(m.PrepareJustification[ii]); size > 3991 {
			err = ssz.ErrBytesLengthFn("Message.PrepareJustification[ii]", size, 3991)
			return
		}
		...
		for _, elem := range m.RoundChangeJustification {
			{
				...
				hh.MerkleizeWithMixin(elemIndx, byteLen, (66770+31)/32)
			}
		}
		...
		for _, elem := range m.PrepareJustification {
			{
				...
				hh.AppendBytes32(elem)
				hh.MerkleizeWithMixin(elemIndx, byteLen, (3991+31)/32)
			}
		}
		hh.MerkleizeWithMixin(subIndx, num, 13)
	}
