...
// Hash: e36cba231fffeacf727f99604fc275e11739cb9e555183c642e409422663f068
// Version: 0.1.3
package qbft
...
func (m *Message) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (8) 'FullData'
	if size := len(m.FullData); size > 5243144 {
		err = ssz.ErrBytesLengthFn("Message.FullData", size, 5243144)
		return
	}
	dst = append(dst, m.FullData...)
...
func (m *Message) UnmarshalSSZ(buf []byte) error {
	...
	// Field (8) 'FullData'
	{
		...
		if cap(m.FullData) == 0 {
			m.FullData = make([]byte, 0, len(buf))
		}
		m.FullData = append(m.FullData, buf...)
	}
	return err
}
...
func (m *Message) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	...
	{
		elemIndx := hh.Index()
		byteLen := uint64(len(m.FullData))
		if byteLen > 5243144 {
			err = ssz.ErrIncorrectListSize
			return
		}
		hh.Append(m.FullData)
