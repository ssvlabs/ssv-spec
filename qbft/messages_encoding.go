...
// Hash: f21f9fc528c2563ab282526daa498e4a40b9ca66171f85b86037c4faf218a328
// Version: 0.1.3
package qbft
...
func (m *Message) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	for ii := 0; ii < len(m.RoundChangeJustification); ii++ {
		if size := len(m.RoundChangeJustification[ii]); size > 50624 {
			err = ssz.ErrBytesLengthFn("Message.RoundChangeJustification[ii]", size, 50624)
			return
		}
	...
	for ii := 0; ii < len(m.PrepareJustification); ii++ {
		if size := len(m.PrepareJustification[ii]); size > 3616 {
			err = ssz.ErrBytesLengthFn("Message.PrepareJustification[ii]", size, 3616)
			return
		}
		...
		for _, elem := range m.RoundChangeJustification {
			{
				...
				hh.MerkleizeWithMixin(elemIndx, byteLen, (50624+31)/32)
			}
		}
		...
		for _, elem := range m.PrepareJustification {
			{
				...
				hh.AppendBytes32(elem)
				hh.MerkleizeWithMixin(elemIndx, byteLen, (3616+31)/32)
			}
		}
		hh.MerkleizeWithMixin(subIndx, num, 13)
	}
