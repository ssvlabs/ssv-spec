...
func RoundRobinProposer(state *State, round Round) types.OperatorID {
	...
	if state.Height != FirstHeight {
		firstRoundIndex += int(state.Height) % len(state.SharedValidator.Committee)
	}

	index := (firstRoundIndex + int(round) - int(FirstRound)) % len(state.SharedValidator.Committee)
	return state.SharedValidator.Committee[index].OperatorID
}
