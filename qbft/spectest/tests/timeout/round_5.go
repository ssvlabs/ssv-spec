...
func Round5() tests.SpecTest {
	...
	pre.State.Round = 5
	pre.State.ProposalAcceptedForCurrentRound = testingutils.TestingProposalMessageWithRound(ks.NetworkKeys[1], types.OperatorID(1), 5)
	...
	return &SpecTest{
		...
		OutputMessages: []*types.SignedSSVMessage{
			testingutils.SignQBFTMsg(ks.NetworkKeys[1], types.OperatorID(1), &qbft.Message{
