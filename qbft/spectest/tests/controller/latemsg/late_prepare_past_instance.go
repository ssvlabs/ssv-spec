...
func LatePreparePastInstance() tests.SpecTest {
	...
	lateMsg := testingutils.TestingPrepareMultiSignerMessageWithHeight([]*rsa.PrivateKey{ks.NetworkKeys[4]}, []types.OperatorID{4}, qbft.FirstHeight)
...
func latePreparePastInstanceStateComparison(height qbft.Height, lateMsg *types.SignedSSVMessage) *comparable.StateComparison {
	ks := testingutils.Testing4SharesSet()
	...
	for i := 0; i <= int(height); i++ {
		...
		instance.State.ProposalAcceptedForCurrentRound = testingutils.TestingProposalMessageWithParams(ks.NetworkKeys[1], types.OperatorID(1), qbft.FirstRound, qbft.Height(i), testingutils.TestingQBFTRootData, nil, nil)
