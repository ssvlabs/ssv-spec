...
func PeerPreparedDifferentHeights() tests.SpecTest {
	...
	msgs := []*types.SignedSSVMessage{
		testingutils.TestingRoundChangeMessageWithRound(ks.NetworkKeys[1], types.OperatorID(1), 3),
		...
		testingutils.TestingRoundChangeMessageWithParams(ks.NetworkKeys[3], types.OperatorID(3), 3, qbft.FirstHeight,
	...
	return &tests.MsgProcessingSpecTest{
		...
		OutputMessages: []*types.SignedSSVMessage{
			testingutils.TestingProposalMessageWithParams(ks.NetworkKeys[1], types.OperatorID(1), 3, qbft.FirstHeight,
