...
func CompareQBFTState(source, target *qbft.State) []testingutilscomparable.Difference {
	...
	if shareDiff := testingutilscomparable.AddExternalCompare("Share", testingutilscomparable.CompareShare(source.Share, target.Share)); len(shareDiff) > 0 {
	...
	if diff := testingutilscomparable.AddExternalCompare("ProposalAcceptedForCurrentRound", CompareQBFTMessage(source.ProposalAcceptedForCurrentRound, target.ProposalAcceptedForCurrentRound)); len(diff) > 0 {
		ret = append(ret, diff)
	}
	...
	if diff := testingutilscomparable.AddExternalCompare("ProposeContainer", CompareQBFTMessageContainer(source.ProposeContainer, target.ProposeContainer)); len(diff) > 0 {
		ret = append(ret, diff)
	}

	if diff := testingutilscomparable.AddExternalCompare("PrepareContainer", CompareQBFTMessageContainer(source.PrepareContainer, target.PrepareContainer)); len(diff) > 0 {
		ret = append(ret, diff)
	}

	if diff := testingutilscomparable.AddExternalCompare("CommitContainer", CompareQBFTMessageContainer(source.CommitContainer, target.CommitContainer)); len(diff) > 0 {
		ret = append(ret, diff)
	}

	if diff := testingutilscomparable.AddExternalCompare("RoundChangeContainer", CompareQBFTMessageContainer(source.RoundChangeContainer, target.RoundChangeContainer)); len(diff) > 0 {
		ret = append(ret, diff)
	}
